...
class SimpleLoginSecure
{
	...
	function login($user_email = '', $user_pass = '') 
	{
		...
		if ($query->num_rows() > 0) 
		{
			...
			$user_data['admin_user_last_logged_in'] = $user_data['user_last_login'];
            unset($user_data['user_last_login']);

			$this->CI->db->simple_query('UPDATE ' . $this->user_table  . ' SET user_last_login = NOW() WHERE user_id = ' . $user_data['user_id']);

			//Set session data
			unset($user_data['user_pass']);
            $user_data['admin_user_id'] = $user_data['user_id'];
            unset($user_data['user_id']);
			$user_data['admin_user'] = $user_data['user_email']; // for compatibility with Simplelogin
            $user_data['admin_user_email'] = $user_data['user_email'];
	...
	function validatePass($user_pass, $user_email = '')
	{
		$this->CI =& get_instance();
		
		if(!$user_email){
			$user_email = $this->CI->session->userdata('admin_user_email');
		}
	...
	function updatePassword($newpwd, $user_email = '')
	{
		if(!$newpwd){
			return false;
		}
		...
		if ($query->num_rows() > 0)
		{
			...
			//Hash user_pass using phpass
			$hasher = new PasswordHash(PHPASS_HASH_STRENGTH, PHPASS_HASH_PORTABLE);
			$user_pass_hashed = $hasher->HashPassword($newpwd);
				
			$this->CI->db->simple_query('UPDATE ' . $this->user_table  . ' SET user_pass = "'. $user_pass_hashed.'" WHERE user_id = ' . $user_data['user_id']);
